...
import { createEventBookingAction } from "@/lib/data/bookings";

const MAX_SEATS_PER_BOOKING = 4;

export function EventSeatSelect({
  eventId,
  totalSeats,
  available,
}: {
  ...
  function confirm() {
    ...
    startTransition(async () => {
      const result = await createEventBookingAction(eventId, seats);
      if (result.error) {
        setError(result.error);
        return;
      }
      router.push(`/events/${eventId}/confirm/${result.bookingId}`);
    });
  }
  ...
  if (available <= 0) {
    return (
      <div className="rounded-2xl border border-danger/30 bg-danger-bg p-4 text-sm text-danger">
        This event bus is fully booked.
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <SeatCapacityBar total={totalSeats} available={available} />
